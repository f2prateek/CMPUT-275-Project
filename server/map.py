...
class Map:
    ...
    def find_path_djisktra(self, locations, dest = None):
        """
        Find an optimized path to travel through the given list.
        ...
        while todo:
            ...
            for neighbour in vertices:
                ...
                if not neighbour in est_min_cost:
                    todo.add(neighbour)
                    est_min_cost[neighbour] = (est_min_cost[current] + cost((current, neighbour)))
                    #est_min_cost[neighbour] = (est_min_cost[current] + least_cost_path((graph,current,neighbour,cost))'cost')
                    parents[neighbour] = current
                elif est_min_cost[neighbour] > (est_min_cost[current] + cost((current, neighbour))):
                #elif est_min_cost[neighbour] > (est_min_cost[current] + least_cost_path((graph,current,neighbour,cost))'cost'):
        ...
        #if there is no specified dest
        del est_min_cost[start]
        dest = min(est_min_cost, key=lambda x: est_min_cost[x])
        return {'cost': est_min_cost[current], 'path': self.reconstruct_path(start, dest, parents)}
